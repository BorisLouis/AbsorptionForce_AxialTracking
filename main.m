...
%file info
file.path  = 'F:\LAIA\2024\SPT\Controls\210325 PS pH 10 Control';
file.ext   = '.ome.tif';
...
trackParam.memory  = 3;
pxSize = 95;
%expTime = 0.01;% in second Now automatically determined
...
for i = 3:size(mainDir,1)
    ...
    if ~isempty(file2Analyze)
        ...
        warning([mainDir(i).folder filesep mainDir(i).name ' did not contain any ' file.ext ' file and is therefore ignored']);
...
%% filter traces
%Keep only traces that were track over a significant volume
maxzPos = max(abs(planePos))*0.75*1000;

idx = [allTraces{:,3}]>maxzPos;
traces2Keep = allTraces(idx,:);
count = sum(idx);
...
%get exposure time from timing between frames % and convert in sec
expTime = mean(diff(traces2Keep{1}.rT))/1000; 
%calculate the speed
...
filename = [file.path filesep 'allTraces.mat'];
save(filename, 'traces2Keep');
...
disp(['Percentage kept in analysis after filtering:' num2str(round((count/size(allTraces,1)*100))) '%'])
